...
def load_unweighted_graph(tbl_name, filename, doreverse, conn):
    ...
    conn.commit()
    with open(filename) as f:
        cur.copy_from(f, tbl_name, columns=('src_id', 'dst_id'))
    if doreverse:
        cur.execute("insert into %s select dst_id, src_id from %s" % (tbl_name, tbl_name))
    conn.commit()  

def load_weighted_graph(tbl_name, filename, conn):
    ...
    conn.commit()
    with open(filename) as f:
        cur.copy_from(f, tbl_name, columns=('src_id', 'dst_id', 'weight'))
    conn.commit()  
